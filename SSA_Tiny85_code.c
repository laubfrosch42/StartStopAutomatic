...
int main (void)
{
	...
	if (!(PINB & (1<<BUTTON_SSA_IN)))
	{
		...
		_delay_ms(100); // to avoid bouncing-effect resulting in immediate switch off by interrupt
	}
	...
	_delay_ms(4000); // a bit of time to collect LED_X_status and wait for car-electronics to be ready
	...
	while (1)
	{
		...
		if (switch_off == 1) {MCUCR = 0b00110000; sleep_mode();}
	}
}
